...
from sqlalchemy import JSON, CheckConstraint, DateTime, String, event
...
class StockMovement(BaseModel, table=True):
    ...
    id: int | None = Field(default=None, primary_key=True, description="在庫移動ID")
    ...
    type: str = Field(
        ...
        sa_column=Column(
            String(10),
            CheckConstraint(
                "type IN ('IN','OUT','ADJUST')", name="ck_stockmovement_type"
            ),
            nullable=False,
        ),
    )
    qty: int = Field(description="数量")
    ref: str | None = Field(default=None, max_length=255, description="参照情報")
...
def receive_before_update(mapper, connection, target) -> None:  # noqa: ARG001
    """更新前にバージョン番号をインクリメント"""
...
def check_stock_balance(mapper, connection, target) -> None:  # noqa: ARG001
    """在庫移動前に在庫残高をチェック"""
    if target.type == "OUT":
        # 出庫の場合は在庫残高をチェック
        ...
        if session:
            ...
            if current_balance < target.qty:
                raise ValueError(
                    f"在庫が不足しています。現在の在庫: {current_balance}, 出庫要求: {target.qty}"
                )


@event.listens_for(Session, "after_flush")
def validate_stock_balance(session: Session, context) -> None:  # noqa: ARG001
    ...
    for instance in session.dirty.union(session.new):
        if isinstance(instance, StockMovement):
            ...
            if balance < 0:
                raise ValueError(
                    f"在庫がマイナスになります。商品ID: {instance.item_id}, 在庫数: {balance}"
                )
