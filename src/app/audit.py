    ...
    try:
        ...
        if settings.audit_stdout:
            ...
            handler = RotatingFileHandler(
                LOG_PATH, maxBytes=10_000_000, backupCount=5, encoding="utf-8"  # 10MB
            )
        ...
        warnings.warn(f"Failed to initialize audit logger: {e}", stacklevel=2)
        logger = logging.getLogger("inventory.audit.nop")
        logger.addHandler(logging.NullHandler())
        _logger = logger
        return logger
...
class AuditContext:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb) -> None:
        ...
        if exc_type is None:
            self.success = True
            audit(
                f"{self.operation}.success", duration_seconds=duration, **self.context
            )
        else:
            self.success = False
...
def cleanup_old_logs(days_to_keep: int = 30) -> int:
    ...
    try:
        ...
        warnings.warn(f"Failed to cleanup old logs: {e}", stacklevel=2)
        return 0
