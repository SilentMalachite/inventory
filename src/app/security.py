...
def require_api_key(
    ...
    if not x_api_key:
        ...
        raise HTTPException(
            status_code=500, detail="Server configuration error: API key required"
        )
...
def require_basic_auth(
    ...
    if credentials is None:
        raise HTTPException(status_code=401, detail="Unauthorized")
    if not (
        secrets.compare_digest(credentials.username, settings.basic_user)
        and secrets.compare_digest(credentials.password, settings.basic_pass)
    ):
        raise HTTPException(status_code=401, detail="Unauthorized")
...
def validate_csrf_or_400(request: Request, token_from_form: str | None) -> None:
    """Validate CSRF token; raise 400 on failure."""
    expected = request.session.get("csrf_token")
    if (
        not expected
        or not token_from_form
        or not secrets.compare_digest(str(token_from_form), str(expected))
    ):
